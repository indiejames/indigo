...
// TestFuzzyRankingExactBasenameFirst verifies that an exact filename match
// ranks above deeper paths that contain the query as a subsequence.
func TestFuzzyRankingExactBasenameFirst(t *testing.T) {
	files := []string{
		"src/middleware/asset-server.ts",
		"src/routers/service-server.ts",
		"examples/server.ts",
		"src/lib/deep/nested/server-transform.ts",
	}
	fp := newTestPicker(files, true)
	fp.setQuery("server.ts")

	if len(fp.filtered) == 0 {
		t.Fatal("expected at least one match for 'server.ts'")
	}
	if fp.filtered[0] != "examples/server.ts" {
		t.Errorf("top result = %q, want %q", fp.filtered[0], "examples/server.ts")
	}
}

// TestFuzzyRankingPrefixBeforeDeep verifies that a basename-prefix match
// ranks above a deeper path match.
func TestFuzzyRankingPrefixBeforeDeep(t *testing.T) {
	files := []string{
		"src/api/user-server-router.ts",
		"src/server.ts",
		"src/lib/some-other.ts",
	}
	fp := newTestPicker(files, true)
	fp.setQuery("server")

	if len(fp.filtered) == 0 {
		t.Fatal("expected matches for 'server'")
	}
	if fp.filtered[0] != "src/server.ts" {
		t.Errorf("top result = %q, want src/server.ts", fp.filtered[0])
	}
}

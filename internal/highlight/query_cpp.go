...
// cppHighlightQuery is ported from Helix's runtime/queries/{c,cpp}/highlights.scm
// (cpp inherits c upstream), concatenated here because go-sitter-forest returns
// the raw upstream query text and this project's query engine doesn't resolve
// Helix's "; inherits:" directives. See THIRD_PARTY_NOTICES.md for source
// commit and license (MPL-2.0).
//
// Changes from upstream: backticks removed from comments (illegal inside a Go
// raw string literal); the C++26 "^^" reflection operator and splice-bracket
// ([: :]) patterns were dropped because the vendored tree-sitter-cpp grammar
// predates that syntax and rejects the query at compile time. Otherwise
// verbatim.

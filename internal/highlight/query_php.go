...
// phpHighlightQuery is ported from Helix's runtime/queries/php/highlights.scm.
// See THIRD_PARTY_NOTICES.md for source commit and license (MPL-2.0).
//
// Changes from upstream: the vendored tree-sitter-php grammar names the
// closing "?>" tag as an anonymous token rather than a (php_end_tag) node, so
// occurrences of (php_end_tag) were replaced with the literal "?>"; the
// "yield from" compound keyword isn't a single token in this grammar version
// and was dropped (plain "yield" is still captured). Otherwise verbatim.
